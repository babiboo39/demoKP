...
pipeline {
    ...
    environment {
        ...
        build_tool = sh (script:  ''' /home/babiboo39/Documents/_private/script/demoKP/testScript.sh specifyBuild ''' , returnStdout: true).trim()
        env_name = sh (script: ''' /home/babiboo39/Documents/_private/script/demoKP/testScript.sh specifyEnv ''', returnStdout: true).trim()
    }
    stages {
        stage ('Build & Test') {
            parallel {
